...
export const JwtRefreshTokenGuard = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      token = jwt.verify(req.headers.authorization as string, jwtConfig.jwt_public_refresh_key, { algorithms: ['RS256'] }) as typeof token
    } catch {
