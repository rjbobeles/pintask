...
describe('[Middleware] JWT Refresh Token Guard', () => {
  ...
  it('should pass with valid token and session', async () => {
    ...
    await JwtRefreshTokenGuard(mockRequest as Request, mockResponse as Response, nextFunction as unknown as NextFunction)

    expect(jwt.verify).toHaveBeenCalledWith('mock-token', jwtConfig.jwt_public_refresh_key, { algorithms: ['RS256'] })
